...
namespace app\modules\customer\classes;
    ...
    function saveCustomer($data)
    {
        ...
        foreach ($requiredFields as $field) {
            if (!isset($data[$field]) || empty($data[$field])) {
                return array("return_code" => false, "return_data" => "$field is missing or empty.");
            }
        }

        // Check for existing customer
        $existingCustomer = "SELECT * FROM Customer WHERE FirstName=:FirstName AND LastName=:LastName AND Email=:Email AND PhoneNumber=:PhoneNumber";
        $existingParams = [
        ...
        $existingCustomerResult = DBController::sendData($existingCustomer, $existingParams);

        if ($existingCustomerResult) {
            return array("return_code" => false, "return_data" => "Customer already exists.");
        }

        // Insert new customer
        $insertCustomer = "INSERT INTO Customer (FirstName, LastName, Email, PhoneNumber,DeliveryAddress) VALUES (:FirstName, :LastName, :Email, :PhoneNumber, :DeliveryAddress)";
        $insertParams = [
            [":FirstName", strip_tags($data["FirstName"])],
            [":LastName", strip_tags($data["LastName"])],
            [":Email", strip_tags($data["Email"])],
            [":PhoneNumber", strip_tags($data["PhoneNumber"])],
            [":DeliveryAddress", strip_tags($data["DeliveryAddress"])]
        ];

        $insertCustomerResult = DBController::ExecuteSQLID($insertCustomer, $insertParams);
        ...
        if ($insertCustomerResult) {
            $user = array(
                ...
                "EmailID" => $data["Email"],
                "UserType" => 4,
                "CustomerID" => $insertCustomerResult,
                "ValidateUsernameOnly" =>  True
            );
            $userdata = (new Signup())->request($user);
            if ($userdata)
                //create new customer users
                return array("return_code" => true, "return_data" => array("Message" => "Successfully signed up to ongyi", "Credentails" => $userdata['message']));
        } else {
            return array("status" => false, "message" => "Failed to register  account");
        }
    }
